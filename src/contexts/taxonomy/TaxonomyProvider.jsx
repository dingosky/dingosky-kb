...
const TaxonomyProvider = ({ children }) => {
  ...
  const conceptEditsRefresh = useCallback(
    async (freshConcept, staleConcept) => {
        ...
        await Promise.all(
          addedChildren
            ...
            .map(async childName => {
              ...
              if (existingChild) {
                if (existingChild.parent !== freshConcept.name) {
                  insertConcept({ ...existingChild, parent: freshConcept.name }, conceptMap, aliasMap)
                }
                return
              }
      ...
      const renamed = freshConcept.name !== staleConcept.name
      const reparented = freshConcept.parent !== staleConcept.parent

      if (renamed && reparented) {
        const freshParent = { ...conceptMap[freshConcept.parent] }
        freshParent.children = freshParent.children.filter(
        ...
        const freshParent = { ...conceptMap[freshConcept.parent] }
        freshParent.children = freshParent.children.filter(child => child !== freshConcept.name)
        freshParent.children.push(freshConcept.name)
        freshParent.children.sort()
        insertConcept(freshParent, conceptMap, aliasMap)
      ...
      const committedConcept =
        getTaxonomyConcept(committedTaxonomy, freshConcept.name) ||
        getTaxonomyConcept(committedTaxonomy, staleConcept.name)

      return { concept: committedConcept, taxonomy: committedTaxonomy }
    },
  ...
  const loadConcept = useCallback(
    async (conceptName, force = false) => {
      ...
      try {
        ...
        return getTaxonomyConcept(committedTaxonomy, conceptName)
      } catch (error) {
        showBoundary(error)
      } finally {
